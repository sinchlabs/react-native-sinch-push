...
export default function DemoScreen() {
  ...
  const subsRef = useRef({ token: undefined, push: undefined, inApp: undefined });
  ...
  const info = useCallback(
    (text) => pushLog({ kind: 'info', at: Date.now(), text }),
    [pushLog],
  );
  ...
  return (
    ...
    >
      ...
      <Text style={styles.subtitle}>
        ...
        {global.__turboModuleProxy ? 'new (TurboModule)' : 'old (bridge)'}
      </Text>

      <View style={styles.statusBox}>
        <Text style={styles.statusLine}>initialized: {initialized ? 'yes' : 'no'}</Text>
        <Text style={styles.statusLine}>
          device token:{' '}
          {deviceToken ? `${deviceToken.type} ${deviceToken.token.slice(0, 12)}…` : 'none'}
        </Text>
      ...
      <Section title="Configuration">
        ...
        <View style={styles.row}>
          {envs.map((e) => (
            ...
            >
              <Text style={[styles.chipText, env === e && styles.chipTextActive]}>
                {e}
              </Text>
            </TouchableOpacity>
          ))}
